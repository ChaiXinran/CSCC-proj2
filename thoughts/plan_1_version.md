...
V6 的范围、共享接口和分组规则已经冻结在：

- `docs/native-v6-scope.md`
- `docs/native-v6-interface.md`
- `docs/native-v6-team-plan.md`

V6 不新增语法，按以下顺序推进：

1. 统一 `ToPrimitive`、`ToNumber`、`ToString`、`ToObject` 和原始值包装对象。
2. 完成 String 构造器、原型方法和 UTF-16 索引语义。
3. 完成 Number、Boolean、Math 和 Error 的原型、描述符与边界行为。
4. 完成 JSON 核心 parse/stringify，再扩展 reviver/replacer。
5. 使用统一转换接口回归 Object、Function 和 Array。
6. 建立零失败、零跳过的 V6 固定门，并扫描对应 Builtins 目录。

Map、Set、RegExp、Date、Promise、Temporal、Intl、TypedArray、Atomics 和 Module
暂不进入 V6，后续根据 Native Test262 增量和距离 60% 的差额决定投入。
