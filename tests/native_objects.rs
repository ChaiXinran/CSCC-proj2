...
fn object_is_uses_same_value_semantics() {
    assert_eq!(
        eval(
            "Object.is(NaN, NaN) + ':' + Object.is(0, -0) + ':' + Object.is(undefined, undefined);"
        ),
        "true:false:true"
    );
}
...
fn delete_property_coerces_primitives_but_rejects_nullish() {
    ...
    assert_eq!(
        eval(
            "var ok = false; try { delete null.a; } catch (e) { ok = e instanceof TypeError; } ok;"
        ),
        "true"
    );
}
...
fn accessor_function_names_include_get_or_set_prefix() {
    assert_eq!(
        eval(
            "class C { get y() { return 1; } } Object.getOwnPropertyDescriptor(C.prototype, 'y').get.name;"
        ),
        "get y"
    );
    assert_eq!(
