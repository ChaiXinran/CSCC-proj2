...
use agentjs::test262::{
    ...
    NATIVE_V7_SCAN_SUITES, NATIVE_V7_TEST_COUNT, NATIVE_V8_SCAN_TEST_COUNT,
    NATIVE_V9_SCAN_TEST_COUNT, NATIVE_V10_SCAN_TEST_COUNT, NATIVE_V11_SCAN_TEST_COUNT,
    RunnerOptions, Status, run,
};
...
#[test]
fn native_v7_scan_selects_the_frontend_cache_safety_suites() {
    let mut options = RunnerOptions::default();
    options.select_native_v7_scan();
...
#[test]
fn native_v9_scan_selects_the_locked_failed_case_manifest() {
    let mut options = RunnerOptions::default();
    options.select_native_v9_scan();

    assert_eq!(options.files.len(), NATIVE_V9_SCAN_TEST_COUNT);
    assert!(options.suites.is_empty());
    assert!(!options.skip_unsupported);
    assert!(options.files.iter().all(|path| path.starts_with("test")));
}
...
#[test]
fn native_v10_scan_selects_the_locked_failed_case_manifest() {
    let mut options = RunnerOptions::default();
    options.select_native_v10_scan();

    assert_eq!(options.files.len(), NATIVE_V10_SCAN_TEST_COUNT);
    assert!(options.suites.is_empty());
    assert!(!options.skip_unsupported);
    assert!(options.files.iter().all(|path| path.starts_with("test")));
}
...
#[test]
fn native_v11_scan_selects_the_locked_failed_case_manifest() {
    let mut options = RunnerOptions::default();
    options.select_native_v11_scan();

    assert_eq!(options.files.len(), NATIVE_V11_SCAN_TEST_COUNT);
    assert!(options.suites.is_empty());
    assert!(!options.skip_unsupported);
    assert!(options.files.iter().all(|path| path.starts_with("test")));
}
