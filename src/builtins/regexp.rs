...
fn translate_js_pattern_for_rust(pattern: &str, flags: &str) -> String {
    ...
    let normalized_surrogate_pairs = pattern.replace(
        r"[\uD800-\uDBFF][\uDC00-\uDFFF]",
        r"[\u{10000}-\u{10FFFF}]",
    );
    ...
    let needs_legacy_class_open =
        !unicode_mode && pattern.chars().filter(|character| *character == '[').count() > 1;
...
mod tests {
    ...
    fn legacy_class_escape_range_treats_the_hyphen_as_a_literal() {
        let regex = compile_regex(r"^[^\s-_]$", "")
            .expect("legacy class-escape range should compile");
