...
```mermaid
flowchart LR
    A[Input<br/>CLI / Test262 / Examples] --> B[Engine]
    B --> C{Backend}
    C -->|Default| D[Native Runtime]
    C -->|Reference| E[Boa Runtime]
    D --> F[Lexer → Parser / AST → Bytecode → VM]
    F --> H[Runtime / Builtins / Heap / GC]
    H --> G[JsValue / Completion]
    E --> G
```
