...
- V9-B first runtime substrate pass is implemented: minimal Promise records,
  ...
  fallback helpers. Focused `tests/native_iteration.rs` passes 5/5; JS-visible
...
- V10-B first runtime substrate pass is implemented: shared ArrayBuffer byte
  ...
  Number-backed element load/store helpers. Focused `tests/native_binary_data.rs`
...
- V11-B first object-key ordering precision fix is implemented:
  ...
  index in shared runtime key ordering. Focused `tests/native_object_keys.rs`
  passes 3/3.
