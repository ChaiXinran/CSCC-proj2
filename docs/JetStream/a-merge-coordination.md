...
- 普通 `LoadName` 由“两次环境链遍历并分配临时 resolution Vec”改为单次 declarative-binding 快速解析。
- 遇到 `with` object environment 时仍回退到原来的 Proxy/observable 路径。
- navier-stokes 1 iteration：优化前 3 次 median 3109 ms；优化后 5 次 median 2116 ms、p90 2191 ms，median 改善约 32%。
- `language/statements/with` focused Test262：127/181，0 跳过。
- 全量 Test262：48,422/53,379，较 48,423 工作区基线少 1 个通过；用户已明确接受该权衡。
- 冲突提示：外部提交 `a82a100 (fix runner)` 已包含此快速路径，后续 B/C 合并如修改 `context.rs` 或 `interpreter.rs` 必须人工合并。
...
- correctness 回退默认优先于性能收益处理；本次 -1 为用户明确批准的例外，后续不得继续扩大。
